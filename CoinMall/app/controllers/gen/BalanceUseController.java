...
public class BalanceUseController extends Controller implements IConst {
    
    public static Result balanceUsePage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<BalanceUse>> msg = BaseController.doGetAll("BalanceUse",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(balance_use.render(msg.data));
        } else {
    ...
    public static Result balanceUseBackendPage() {
        return ok(balance_use_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_BalanceUse")
    @Role("create_balance_use")
    public static Result add() {
        Msg<BalanceUse> msg = new Msg<>();

        Form<BalanceUseParser> httpForm = form(BalanceUseParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            BalanceUseParser formObj = httpForm.get();            
            BalanceUse newObj = new BalanceUse();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("BalanceUse", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("BalanceUse", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("balance_use_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_BalanceUse")
    @Role("update_balance_use")
    public static Result update(long id) {
        Msg<BalanceUse> msg = new Msg<>();

        BalanceUse found = BalanceUse.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("BalanceUse", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("BalanceUse", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class BalanceUseParser {
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<BalanceUse>> msg = BaseController.doGetAll("BalanceUse",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, BalanceUse.find.findRowCount());
		List<BalanceUse> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(BalanceUse.class) + "报表");
		// 设置列
        ...
        sheet2.setDefaultColumnStyle(2, cellStyle);//coin
        sheet2.setColumnWidth(3, 4000);
        sheet2.setDefaultColumnStyle(3, cellStyle);//money
        sheet2.setColumnWidth(4, 4000);
        sheet2.setDefaultColumnStyle(4, cellStyle);//rate
        ...
        sheet2.setDefaultColumnStyle(8, cellStyle);//comment


		// 创建表头
		HSSFRow title = sheet2.createRow(0);
		title.setHeightInPoints(50);
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(BalanceUse.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "user"));//user_id
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "coin"));//coin
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "money"));//money
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "rate"));//rate
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "status"));//status
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(7).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "createdAt"));//created_at
        titleRow.createCell(8).setCellValue(TableInfoReader.getFieldComment(BalanceUse.class, "comment"));//comment
		...
		for (i = 0; i < list.size(); i++) {
            ...
            cell2.setCellValue(item.coin);
            HSSFCell cell3 = row.createCell(3);
            cell3.setCellValue(item.money);
            HSSFCell cell4 = row.createCell(4);
            cell4.setCellValue(item.rate);
            HSSFCell cell5 = row.createCell(5);
            cell5.setCellValue(EnumInfoReader.getEnumName(BalanceUse.class, "status", item.status));
            ...
            if (item.comment == null) {
                ...
                cell8.setCellValue(item.comment);
            }
		}
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_BalanceUse")
    @Role("report_balance_use")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(BalanceUse.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
