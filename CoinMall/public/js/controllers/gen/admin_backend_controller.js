var app = angular.module('AdminBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);
...
app.controller('AdminBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    for (var i = 0; i < Object.keys(objEnumInfo.Admin.userRoleEnum).length; i++) {
        $scope.userRoleEnum.push({"id": i, "name": objEnumInfo.Admin.userRoleEnum[i]})
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.Admin.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'password', displayName: objFieldInfo.Admin.password, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'descriptions', displayName: objFieldInfo.Admin.descriptions, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastUpdateTime', displayName: objFieldInfo.Admin.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.Admin.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastLoginIp', displayName: objFieldInfo.Admin.lastLoginIp, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastLoginTime', displayName: objFieldInfo.Admin.lastLoginTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastLoginIpArea', displayName: objFieldInfo.Admin.lastLoginIpArea, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'status', displayName: objFieldInfo.Admin.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateAdminStatus},
        {field: 'userRoleEnum', displayName: objFieldInfo.Admin.userRoleEnum, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateAdminUserRoleEnum},
        {field: 'comment', displayName: objFieldInfo.Admin.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/admin?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'admin_backend_' + getNowFormatDate()
