var app = angular.module('BalanceUseBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);

var checkboxTemplateBalanceUse = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" ng-click="grid.appScope.updateEntity(col, row)" /></div>';
...
app.controller('BalanceUseBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isBalanceUse = true;
    ...
    dropdownTemplateBalanceUseStatus = '<div>' +
        '<select ng-model="MODEL_COL_FIELD" ' +
        'ng-change="grid.appScope.updateEntity(col, row)" style="padding: 2px;">'
    for (var i = 0; i < Object.keys(objEnumInfo.BalanceUse.status).length; i++) {
        $scope.status.push({"id": i, "name": objEnumInfo.BalanceUse.status[i]})
        dropdownTemplateBalanceUseStatus += '<option ng-selected="MODEL_COL_FIELD==' + i
            + '" value=' + i + '>' + objEnumInfo.BalanceUse.status[i] + '</option>'
    }
    dropdownTemplateBalanceUseStatus += '</select></div>'
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.BalanceUse.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'refUserId', displayName: objFieldInfo.BalanceUse.refUserId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/user?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},
        {field: 'coin', displayName: objFieldInfo.BalanceUse.coin, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'money', displayName: objFieldInfo.BalanceUse.money, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'rate', displayName: objFieldInfo.BalanceUse.rate, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'status', displayName: objFieldInfo.BalanceUse.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateBalanceUseStatus},
        {field: 'lastUpdateTime', displayName: objFieldInfo.BalanceUse.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.BalanceUse.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.BalanceUse.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},

        {field: 'user', displayName: objFieldInfo.BalanceUse.user, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/user?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
    ];
    ...
    function refreshData(showMsg){
        var url = '/base/BalanceUse/all?page=' 
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/BalanceUse/' + content.id
        ...
        $http.get('/new/balanceuse')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    ...
                    createDialogService("/assets/html/edit_templates/balance_use.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/balanceuse?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'balance_use_backend_' + getNowFormatDate()
