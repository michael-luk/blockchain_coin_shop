var app = angular.module('ReturnInfoBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);

var checkboxTemplateReturnInfo = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" ng-click="grid.appScope.updateEntity(col, row)" /></div>';
...
app.controller('ReturnInfoBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isReturnInfo = true;
    ...
    dropdownTemplateReturnInfoStatus = '<div>' +
        '<select ng-model="MODEL_COL_FIELD" ' +
        'ng-change="grid.appScope.updateEntity(col, row)" style="padding: 2px;">'
    for (var i = 0; i < Object.keys(objEnumInfo.ReturnInfo.status).length; i++) {
        $scope.status.push({"id": i, "name": objEnumInfo.ReturnInfo.status[i]})
        dropdownTemplateReturnInfoStatus += '<option ng-selected="MODEL_COL_FIELD==' + i
            + '" value=' + i + '>' + objEnumInfo.ReturnInfo.status[i] + '</option>'
    }
    dropdownTemplateReturnInfoStatus += '</select></div>'
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.ReturnInfo.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'contact', displayName: objFieldInfo.ReturnInfo.contact, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'phone', displayName: objFieldInfo.ReturnInfo.phone, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'refUserId', displayName: objFieldInfo.ReturnInfo.refUserId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/user?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},
        {field: 'refPurchaseId', displayName: objFieldInfo.ReturnInfo.refPurchaseId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/purchase?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},
        {field: 'status', displayName: objFieldInfo.ReturnInfo.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateReturnInfoStatus},
        {field: 'returnType', displayName: objFieldInfo.ReturnInfo.returnType, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'shipNo', displayName: objFieldInfo.ReturnInfo.shipNo, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<div><a href="https://www.kuaidi100.com/chaxun?com=&nu={{COL_FIELD}}" target="_blank">{{COL_FIELD}}</a></div>'},
        {field: 'lastUpdateTime', displayName: objFieldInfo.ReturnInfo.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.ReturnInfo.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.ReturnInfo.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},

        {field: 'user', displayName: objFieldInfo.ReturnInfo.user, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/user?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
        {field: 'purchase', displayName: objFieldInfo.ReturnInfo.purchase, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/purchase?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
    ];
    ...
    function refreshData(showMsg){
        var url = '/base/ReturnInfo/all?page=' 
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/ReturnInfo/' + content.id
        ...
        $http.get('/new/returninfo')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    ...
                    createDialogService("/assets/html/edit_templates/return_info.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/returninfo?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'return_info_backend_' + getNowFormatDate()
