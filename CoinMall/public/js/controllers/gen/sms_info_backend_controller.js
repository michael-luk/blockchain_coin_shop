var app = angular.module('SmsInfoBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);

var checkboxTemplateSmsInfo = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" ng-click="grid.appScope.updateEntity(col, row)" /></div>';
...
app.controller('SmsInfoBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isSmsInfo = true;
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.SmsInfo.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'phone', displayName: objFieldInfo.SmsInfo.phone, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'checkCode', displayName: objFieldInfo.SmsInfo.checkCode, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'sendXml', displayName: objFieldInfo.SmsInfo.sendXml, width: '100', headerTooltip: '点击表头可进行排序', enableCellEdit: true, cellTemplate: '<div ng-bind-html="COL_FIELD | safehtml"></div>'},
        {field: 'returnTable', displayName: objFieldInfo.SmsInfo.returnTable, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'receiveXml', displayName: objFieldInfo.SmsInfo.receiveXml, width: '100', headerTooltip: '点击表头可进行排序', enableCellEdit: true, cellTemplate: '<div ng-bind-html="COL_FIELD | safehtml"></div>'},
        {field: 'code', displayName: objFieldInfo.SmsInfo.code, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'returnMsg', displayName: objFieldInfo.SmsInfo.returnMsg, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastUpdateTime', displayName: objFieldInfo.SmsInfo.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.SmsInfo.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.SmsInfo.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/SmsInfo/' + content.id
    ...
    $scope.addContent = function(){
        $http.get('/new/smsinfo')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    $scope.currentObj = data.data;
                    
                    
                    createDialogService("/assets/html/edit_templates/sms_info.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/smsinfo?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'sms_info_backend_' + getNowFormatDate()
