var app = angular.module('ConfigBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);
...
app.controller('ConfigBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    for (var i = 0; i < Object.keys(objEnumInfo.Config.typeEnum).length; i++) {
        $scope.typeEnum.push({"id": i, "name": objEnumInfo.Config.typeEnum[i]})
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.Config.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'content', displayName: objFieldInfo.Config.content, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'typeEnum', displayName: objFieldInfo.Config.typeEnum, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateConfigTypeEnum},
        {field: 'lastUpdateTime', displayName: objFieldInfo.Config.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.Config.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.Config.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/config?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'config_backend_' + getNowFormatDate()
